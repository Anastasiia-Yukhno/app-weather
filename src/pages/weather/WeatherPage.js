...
import {AddButton, AddNewCity, CardContainer, InputCityName, Wrapper} from "./weatherPage.styled";
...
export const WeatherPage = () => {
    ...
    useEffect( () => {
        CityList.map( async (city) => {
             await fetch(apiRequest(city)).then(function (response) {
    ...
    const checkAndAdd = async () => {
        ...
        await fetch(apiRequest(cityName)).then(function (response) {
            return response.json()
        })
            .then(function (data) {
                if(data.cod === '404') {
                ...
                newCardList.push(data)
                window.data = data
            }).then(() => setCardList([...newCardList])).catch((e) => {
                console.log('Error', e)
